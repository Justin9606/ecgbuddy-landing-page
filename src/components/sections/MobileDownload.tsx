...
interface MobileDownloadProps {
  isAdminView?: boolean;
  content?: MobileDownloadContent;
}

const MobileDownload: React.FC<MobileDownloadProps> = ({ isAdminView = false, content }) => {
  const [activeApp, setActiveApp] = useState<string>("ios");
  const [mobileContent, setMobileContent] = useState<MobileDownloadContent | null>(content || null);

  // Load content from admin or use defaults (only if not in admin view)
  React.useEffect(() => {
    if (isAdminView || content) {
      setMobileContent(content || null);
      return;
    }

    const loadContent = () => {
      try {
        const loadedContent = getSectionContent<MobileDownloadContent>('mobileDownload');
        setMobileContent(loadedContent);
      } catch (error) {
        console.error('Error loading mobile download content:', error);
  ...
  if (!mobileContent && !isAdminView) {
    return (
      <section className="relative py-32 overflow-hidden">
        <div className="text-center">
          <div className="w-8 h-8 border-2 border-slate-300 border-t-slate-600 rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-slate-600">Loading mobile download content...</p>
        </div>
      </section>
    );
  }
  ...
  const defaultContent: MobileDownloadContent = {
    sectionHeader: {
      title: "Take ECG Buddy everywhere you go",
      description: "Access powerful ECG analysis on any device.",
    },
    apps: [],
    bottomFeatures: [],
  };

  const activeContent = mobileContent || defaultContent;
  ...
  return (
    <section id="mobile-download" className={`relative ${isAdminView ? 'py-16' : 'py-32'} overflow-hidden`}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        {/* Main Content */}
        <div className={`grid grid-cols-1 ${isAdminView ? 'lg:grid-cols-1' : 'lg:grid-cols-2'} gap-16 items-center`}>
          ...
          >
            ...
            {apps.length > 1 && (
              ...
              >
                <div className={`grid ${apps.length === 2 ? 'grid-cols-2' : 'grid-cols-3'} gap-2`}>
                  {apps.map((app, index) => {
        ...
        {activeContent.bottomFeatures.length > 0 && (
          <motion.div
            ...
            viewport={{ once: true }}
          >
            {activeContent.bottomFeatures.map((feature, index) => {
