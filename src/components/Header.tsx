                ...
                <div key={key} className="relative">
                  <button 
                    ...
                    className={`flex items-center space-x-1 px-4 py-2 text-slate-700 hover:text-slate-900 transition-all duration-200 font-medium rounded-lg hover:bg-red-50/50 focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:bg-red-50/50 ${
                      activeDropdown === key ? 'bg-red-50/50 text-slate-900' : ''
                    }`}
                  >
                    <span>{key}</span>
                      ...
                      <div className="relative grid grid-cols-3 gap-8">
                            ...
                            <div key={sectionIndex}>
                              ...
                              <div className="space-y-1">
                                {section.items.map((item, itemIndex) => (
                                  ...
                                  >
                                    <div className={`w-8 h-8 bg-gradient-to-br ${item.gradient} rounded-lg flex items-center justify-center flex-shrink-0 group-hover/item:scale-110 transition-transform duration-200 shadow-sm`}>
                        ...
                        <div className="bg-gradient-to-br from-slate-50 to-slate-100/80 rounded-2xl p-6 border border-slate-200/60 relative overflow-hidden group/cta">
                          ...
                          <div className="relative text-center">
                            ...
                            <h3 className="font-semibold text-slate-900 mb-2">{menu.cta.title}</h3>
                            <p className="text-sm text-slate-600 mb-6 leading-relaxed">{menu.cta.description}</p>
                            <a
                              href={menu.cta.link}
                              onClick={() => setActiveDropdown(null)}
          ...
          <div className={`lg:hidden transition-all duration-500 ease-out ${
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-[0_20px_70px_rgba(0,0,0,0.15)] mt-4 p-6 max-h-[70vh] overflow-y-auto">
              ...
              {Object.entries(megaMenuItems).map(([key, menu], index) => (
                <div key={key} className={`${index > 0 ? 'mt-6 pt-6 border-t border-slate-100' : ''}`}>
                    ...
                    <div key={sectionIndex} className="mb-4">
                      ...
                      <div className="space-y-1">
                        {section.items.map((item, itemIndex) => (
                          ...
                          >
                            <div className={`w-8 h-8 bg-gradient-to-br ${item.gradient} rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm`}>
                              <item.icon className="w-4 h-4 text-white" />
                            </div>
